...
![image-20200704151731216](/1_内存与垃圾回收篇/1_JVM与Java体系结构images/image-20200704151731216.png)
...
在宏观上：俄们可以同时打开多 个应用程序，每个程序并行不悖，同时运行。
...
因此一般来说（由字节码中跟随o指令所决定），new指令之后会接着就是执行方法，把对象按照程序员的意愿进行初始化，这样一个真正可用的对象才算完成创建出来。
