...
public class HashMap<K,V> extends AbstractMap<K,V>
     ...
     *
     * 作用：返回一个大于当前值cap的一个数字，这个数字一定是2的次方
     *
     * cap = 10
     * n = 10 - 1 => 9
    ...
    public HashMap(int initialCapacity, float loadFactor) {
        ...
        //做了一些校验，initialCapacity 必须 > 0,最大值也只能为MAXIMUM_CAPACITY
     ...
     * @param onlyIfAbsent if true, don't change existing value   是否存在重复key
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
        ...
        //真正的建表，延迟初始化，第一次调用putVal会初始化最消耗内存的散列表
        if ((tab = table) == null || (n = tab.length) == 0)
            n = (tab = resize()).length;

        //最简单情况，寻址找到的插位刚好是null，这个时候直接将当前 k-v=>node 扔进去就好了
        if ((p = tab[i = (n - 1) & hash]) == null)
            tab[i] = newNode(hash, key, value, null);


        else {
            //e:node临时元素，不为null的话，找到一个与当前插入的key-value一致的key的元素
            //k:表示临时的key
            Node<K,V> e; K k;

            //表示桶位中的该元素，与你插入的元素key完全一致，表示后续需要进行替换操作
            if (p.hash == hash &&
